...
export function WinningContests() {
  ...
  if (!user) return null;
  const winningContests = MOCK_CONTESTS.filter(contest => contest.winnerId === user.id);
  const totalWinnings = winningContests.reduce((sum, contest) => sum + contest.prizeMoney, 0);
  return <div className="space-y-8">
      ...
      {/* Stats Card */}
      <div className="bg-gradient-to-r from-yellow-600/20 to-orange-600/20 border border-yellow-500/30 rounded-2xl p-8 flex items-center justify-between">
        <div>
      ...
      {winningContests.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {winningContests.map(contest => <div key={contest.id} className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden group hover:border-yellow-500/50 transition-colors">
